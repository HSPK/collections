...
A drag, including many pointer moves, is one undo step. Escape/pointer cancellation restores its starting geometry. Native text-field undo is not intercepted. Ctrl/Cmd+Z outside fields and the visible undo/redo buttons operate experiment history. Preset loads, reset and successful imports are also undoable.
...
Targeted numerical and browser workflows: `SITE_URL=http://127.0.0.1:4173 npx playwright test tests/projects/lumen.spec.ts`. Desktop and mobile screenshots are written to the individual test artifact directories.
